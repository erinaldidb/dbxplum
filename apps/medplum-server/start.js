...
async function fetchDatabricksToken() {
  ...
  if (!clientId || !clientSecret || !host) {
    console.log('[auth] No Databricks SP credentials found, skipping OAuth token fetch');
    return { token: undefined, expiresIn: 0 };
  }

  const tokenUrl = `https://${host}/oidc/v1/token`;
  console.log(`[auth] Fetching OAuth token from ${tokenUrl}...`);
  ...
  if (!resp.ok) {
    ...
    console.error(`[auth] OAuth token fetch failed: ${resp.status} ${text}`);
    return { token: undefined, expiresIn: 0 };
  }

  const data = await resp.json();
  console.log(`[auth] OAuth token obtained (${data.access_token.length} chars, expires_in: ${data.expires_in}s)`);
  return { token: data.access_token, expiresIn: data.expires_in };
}

function startTokenRefreshLoop(initialExpiresIn) {
  if (!initialExpiresIn) return;
  ...
  function scheduleRefresh(expiresIn) {
    const delayMs = Math.max((expiresIn - BUFFER_SECONDS) * 1000, 60_000);
    console.log(`[token-refresh] Next refresh scheduled in ${Math.round(delayMs / 1000)}s`);

    setTimeout(async () => {
      let retries = 0;
      while (retries < MAX_RETRIES) {
        try {
          const { token, expiresIn: newExpiresIn } = await fetchDatabricksToken();
          if (!token) throw new Error('Token fetch returned undefined');

          process.env.PGPASSWORD = token;
          generateConfig(token);
          console.log(`[token-refresh] Token refreshed successfully (expires_in: ${newExpiresIn}s)`);
          scheduleRefresh(newExpiresIn);
          return;
        } catch (err) {
          retries++;
          console.error(`[token-refresh] Refresh attempt ${retries}/${MAX_RETRIES} failed: ${err.message}`);
      ...
      console.error('[token-refresh] All retries exhausted, scheduling next attempt at half remaining interval');
      const fallbackDelay = Math.max(Math.round((expiresIn - BUFFER_SECONDS) / 2), 60);
      scheduleRefresh(fallbackDelay);
    }, delayMs);
  }
...
function generateConfig(pgToken) {
  ...
  const config = {
    ...
    database: {
      ...
      password: pgToken,  // OAuth JWT from Databricks SP
      runMigrations: true,
